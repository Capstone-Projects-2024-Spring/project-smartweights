...
class UserFitnessPlanManager : ObservableObject{
    @Published var userFitnessPlan: UserFitnessPlanModel?
    let CKManager = CloudKitManager()
    
    func createUserFitnessPlan(numberWorkoutDays: Int64, user: CKRecord.Reference, weightGoal: Int64){
        let userFitnessPlan = UserFitnessPlanModel(numberWorkoutDays: numberWorkoutDays, user: user, weightGoal: weightGoal)
